...
if __name__ == "__main__":
    ...
    dataset = OurDataset(os.path.join(args.data_root), device, near=args.near, far=args.far, img_scale=0.25)
    H, W = dataset.H, dataset.W

    vol_dims, vol_origin, voxel_size = get_volume_setting(args)
    tsdf_volume = TSDFVolumeTorch(vol_dims, vol_origin, voxel_size, device, margin=3, fuse_color=args.fuse_color)

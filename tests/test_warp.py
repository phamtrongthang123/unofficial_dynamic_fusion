...
import time 
from tmp_utils import SE3, decompose_se3, cal_dist, uniform_sample, SE3_novmap, custom_transpose_batch
import numpy as np 
...
def test_GN_blendingW():
    ...
    def energy(Xc, Tlw, dqs, dgw, dgv, target):
        ...
        # print(T.shape, T)
        R, t= decompose_se3(T)
        Xt =  (torch.einsum('bij,j->bi', R, Xc) + t.squeeze(-1)).squeeze() # shape [3] == target 
        # print(R,t, R.shape, t.shape)
        return torch.linalg.norm(Xt-target), torch.linalg.norm(Xt-target)**2
    ...
    for i in range(10):
        ...
        A = tmp_A.view(bs*knn,8,8)  # + res term # [bs,knn,8,8]
        ...
        solved_delta = torch.linalg.lstsq(A, b) # [bs*knn,8,1]
        dqs -= solved_delta.solution.view(dqs.shape)
        print(fx)
    # print(dqs)
    R,t = decompose_se3(SE3_novmap(dqs.view(bs*knn,8)))
    assert 0 == 1